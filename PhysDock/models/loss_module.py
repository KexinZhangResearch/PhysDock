...
def smooth_lddt_loss(
    ...
    expect_angle_mask = torch.matmul(expect_bond_mask.type(diff_gt.dtype), expect_bond_mask.type(diff_gt.dtype)) * base_mask
    #########################################
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            if loss_name != "smooth_lddt_loss":
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    ...
                    loss = torch.zeros_like(loss, requires_grad=True)
                cum_loss = cum_loss + weight * loss
                cum_loss = cum_loss + 1.0 * ((expect_bond_loss * 10) ** 2 + (expect_bond_loss * 10))
                cum_loss = cum_loss + 0.5 * ((expect_angle_loss * 10) ** 2 + (expect_angle_loss * 10))
                losses[loss_name] = loss.detach().clone()
                losses["expect_bond_loss"] = expect_bond_loss.detach().clone()
                losses["expect_angle_loss"] = expect_angle_loss.detach().clone()
