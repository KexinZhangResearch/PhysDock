...
def clamp_distance_loss(
    ...
    # diff_mask = ((atom_id_to_token_id[...,None]==atom_id_to_token_id[...,None,:])
    #              *x_exists[..., None] * x_exists[..., None, :])[...,None,:,:]

    # loss = ((t_hat ** 2 + sigma_data ** 2) / (t_hat + sigma_data) ** 2) * masked_mean(
    #     diff_mask, (diff_denoised - diff_gt[...,None, :, :]).abs(),dim=(-1, -2))
    # Close to smooth_lddt loss
    loss = masked_mean(
        diff_mask, (diff_denoised - diff_gt[..., None, :, :]).abs(), dim=(-1, -2))
    return loss.mean()
...
def smooth_lddt_loss(
    ...
    expect_bond_mask = ((diff_gt < 2) * x_exists[..., None] * x_exists[..., None, :])[..., None, :, :]
    num_atoms = len(x_gt)
    base_mask = (1 - torch.eye(num_atoms, dtype=diff_gt.dtype,
                               device=expect_bond_mask.device))[..., None, :, :]
    expect_bond_mask = expect_bond_mask * base_mask

    expect_angle_mask = torch.matmul(expect_bond_mask.type(diff_gt.dtype),
                                     expect_bond_mask.type(diff_gt.dtype)) * base_mask
    #########################################
    # TODO: expect angle loss               #
    #########################################
    ...
    expect_bond_loss = masked_mean(expect_bond_mask, delta, dim=(-1, -2))
    expect_angle_loss = masked_mean(expect_angle_mask, delta, dim=(-1, -2))
    return loss.mean(), expect_bond_loss.mean(), expect_angle_loss.mean()
...
def plddt_loss(
    ...
    ##############################
    diff_pred = torch.norm(x_pred[..., None, :] - x_pred[..., None, :, :], dim=-1)
    diff_gt = torch.norm(x_gt[..., None, :] - x_gt[..., None, :, :], dim=-1)
    ...
    expect_angle_mask = torch.matmul(expect_bond_mask.type(diff_gt.dtype),
                                     expect_bond_mask.type(diff_gt.dtype)) * base_mask
    delta = (diff_pred - diff_gt[..., None, :, :]).abs()
    pred_bond_loss = masked_mean(expect_bond_mask, delta, dim=(-1, -2)).mean()
    pred_angle_loss = masked_mean(expect_angle_mask, delta, dim=(-1, -2)).mean()
    # pred_bond_loss = torch.zeros_like(mean,requires_grad=True)
    # pred_angle_loss = torch.zeros_like(mean,requires_grad=True)
    ##############################
    return mean, pred_bond_loss, pred_angle_loss
...
class RFFoldLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.use_mini_rollout = config.use_mini_rollout
        self.config = config.model.loss
    ...
    def forward(self, outputs, feats):
        loss_fns = {
            ...
            # "clamp_distance_loss": lambda: clamp_distance_loss(
            #     **outputs, **feats, **self.config.clamp_distance_loss,
            # ),
        ...
        for loss_name, loss_fn in loss_fns.items():
            ...
            if loss_name == "smooth_lddt_loss":
                loss, expect_bond_loss, expect_angle_loss = loss_fn()
                ...
                losses["denoised_bond_loss"] = expect_bond_loss.detach().clone()
                losses["denoised_angle_loss"] = expect_angle_loss.detach().clone()
                ...
                cum_loss = cum_loss + weight * loss
                losses[loss_name] = loss.detach().clone()
                losses["mini_rollout_bond_loss"] = expect_bond_loss.detach().clone()
                losses["mini_rollout_angle_loss"] = expect_angle_loss.detach().clone()
            else:
                loss = loss_fn()
