...
****************************************************************************''')
    ...
    assert os.path.isdir(input_pkls_dir)
    if output_dir is None:
        output_dir = os.path.join(os.path.split(input_pkls_dir)[0], os.path.split(input_pkls_dir)[1] + "_predictions")
    ...
    pkls = find_files(input_pkls_dir, "pkl.gz")

    def _collator_fn(pkls):
        pkl = pkls[0]
        try:
            tensors, infer_meta_data = feature_loader.load(system_pkl_path=pkl)

            return pkl, tensors, infer_meta_data
        except Exception as e:
            print(pkl, e)
            return pkl, None, None
    ...
    print(f"# Total Systems: {len(pkls)}")
    ...
    for batch_id, (pkl, tensors, infer_meta_data) in tqdm.tqdm(enumerate(dataloader), total=len(pkls)):
        if infer_meta_data is None:
            continue
        for k, v in tensors.items():
            tensors[k] = v.to(device)
        try:
            sample_id = infer_meta_data["system_id"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', type=str, help='system pkls dir', required=True)
